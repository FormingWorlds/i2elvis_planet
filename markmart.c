...
void pebbleaccr(){ 
	...
	/* Only execute routine if pebble routine is actived (pebble_events > 0) */
	if(pebble_events>0 && pebble_time[1]<currentTime){

		/* Read in all lines from pebble history which were not accreted yet, calculate total pebble mass to be accreted */ 
    		while((pebble_time[mmp]<=currentTime) && (pebble_mass[mmp] > 0.0)){
			if(pebble_time[mmp]>time_last){
    				dMM += pebble_mass[mmp];		/* [kg] */
				printf("Newly accreted pebbles at %f Myr, mass: %f M_Ceres. \n",pebble_time[mmp]/1.0e+6,pebble_mass[mmp]/M_Ceres);
			}
			++mmp;
    		}
		printf("Total pebble mass accreted: %f, at time: %f Myr \n",dMM/M_Ceres,pebble_time[mmp]/1.0e+6);
		...
		/* Save information for next iterations in external file, available after restarts */
