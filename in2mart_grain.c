...
// JWE edit, add an argument to control iron implantation
float fe_vol_frac = 0.;
if (argc != 2) {
	printf("!!! Missing iron volume fraction argument!\n");
	return 1;
}
else {
	fe_vol_frac = atof(argv[1]);
	printf("> Converting %.0f%% of wet silicates into iron\n",fe_vol_frac*100.);
}
...
// JWE edit, randomly distribute iron grains throughout 
// Most viable place to put marker modification
printf("> JWE Edit, implanting iron grains\n");
// Initialise random number generator with current CPU time
srand(time(NULL));
// Initialise counters for wet silicates and iron replacements
int nwets = 0;
int ndrys = 0;
for (int mmmod = 0; mmmod < marknum; mmmod++)
{
	// Check to see if marker is "wet" silicates
	if (markt[mmmod] == 6) {
		// Generate a new random number
		int rand_int = rand();
		// Calculate the random number fraction 
		float rand_fr = (double)(rand_int) / (double)(RAND_MAX);
		// If random number is below volume fraction then we convert this marker to iron
		// It *should* be the case that nothing has been done with this yet
		if (rand_fr <= fe_vol_frac)
		{
			markt[mmmod] = 7;
			double x = (double)(markx[mmmod])/xsize;
			double y = (double)(marky[mmmod])/ysize;
			ndrys++;
			// printf("Converted iron grain at x = %.3e, y = %.3e\n",x,y);
		}
		nwets++;
	}
}
float convfrac = ((double)(ndrys) / (double)(nwets)) * 100.0;
printf("> Converted %d cells out of %d, or %.2f%% by volume\n",ndrys,nwets,convfrac);
printf("> Grains implanted\n");
