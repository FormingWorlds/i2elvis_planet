...
/* written by Gregor J. Golabek (last updated: 18/07/2016) */
int grain()
{
	long int mm1;
	/**/
	...
	for (mm1=0;mm1<marknum;mm1++)
		...
		/* Reset marker type in sector of circle after first impact event */
        	if((timesum+timestep)>=(impact_time[no1]*1.000e+6*365.250*24.000*3600.000))
			{
			...
			if(distancee>=radius_core && distancee<=radius_planet)
				...
				/**/
				/* Reset marker type in sector if interest */
