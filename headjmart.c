...
float val0[MAXMAT],fre0[MAXPAR],bufv[MAXPAR];
long int lin0[MAXMAT];
long int num0[MAXPAR],cur0[MAXPAR];
...
/* ia[] - first column of each row */
/* ja[] - column position of each koef in the respective row from left to right */
...
/* kt[]                - Thermal conductivity koef, W/m/K */
/* ht[]                - Heat Sources, W/m3 */
/* vx[],vy[]           - Cur circle Vx,Vy,  m/sek */
/* tk0[]               - Last circle TK */
/* bondm[]             - boundary position in bondn[],bondv[] for cur par (0) - no bondary */
...
/* pr[]                - pressure, Pa */
/* gp[]                - gravitational potential, J/kg */
/* ga[]                - magnitude of gravity acceleration, m/s^2 */
...
double exx[MAXNOD],eyy[MAXNOD],exy[MAXNOD],esp[MAXNOD];
...
/* markk[]          - temperature [K] for marker */
/* markw[]          - water contents wt% for marker */
/* markd[]          - density [kg/m^3] for marker */
...
/* fl0in[]           - input data All file names */
/* fl0itp[]          - input data files types */
...
/* GXKOEF,GYKOEF                      - Gravitation g for X,Y directions, m/s^2 */
/* tmp_ambient			      		  - Ambient temperature of sticky air */
/* delta_tmp			      		  - Ambient temperature drop from day- to night-side for tide-locked planets */
/* al2627_init,fe6056_init            - Initial abundances of 26Al (e-5) and 60Fe (e-8) relative to their stable isotopes */
...
/* bondnum                            - boundary condition equation num */
/* nodenum3                           - nodenum*3 */
...
/* allinter1() - Vx,Vy,P,T,EPS,SIG calc for marker by interpolation */
/* spncalc()   - vorticity tensor calc */
void ffscanf();
void ffscanf1();
void loadconf();
void loader();
...
/**/
/* <move.c> FILE */
/* viterate()               - General subroutine for Vx,Vy,P Calc by iterative method */
...
/* depthp()               - P calc as the function of depth under the surface */
/* nodewt()               - Weights for horizontal and vertical nodes calculation for marker interpolation */ 
...
double M_diapir,M_diapir_ol,M_diapir_old;            /* New and old masses of the iron diapir pairs [kg] */
double d_iron,d_iron_ol,d_iron_old;                  /* Resulting old and new iron pond thicknesses [m] */
...
double pebble_time[10000];                /* Accretion time of pebbles [Myr] */
